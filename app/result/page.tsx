...
function ResultContent() {
  ...
  const typeCode = [
    getScore('time') >= 0 ? 'A' : 'S',
    getScore('relations') >= 0 ? 'O' : 'S',
    getScore('cognition') >= 0 ? 'I' : 'S',
    getScore('interest') >= 0 ? 'T' : 'S',
    getScore('activity') >= 0 ? 'F' : 'S',
    getScore('values') >= 0 ? 'V' : 'S',
  ].join('');
    ...
    <div className="min-h-screen bg-slate-50 py-12 px-4">
      <div className="max-w-xl mx-auto bg-white rounded-3xl shadow-xl border border-slate-100 p-8">
          ...
          {Object.entries(percents).map(([key, percent]) => (
            <div key={key}>
              <div className="flex justify-between text-xs font-bold text-slate-500 mb-1">
                ...
                <span>{PARAM_INFO[key].label}</span>
        ...
        <div className="bg-slate-50 p-6 rounded-2xl border border-slate-100 mb-8">
          <h3 className="font-bold text-slate-800 text-center mb-4 text-xs">サイレント要因分析</h3>
          <div className="flex justify-center gap-8">
            <div className="relative w-20 h-20">
              {[ {key: 'time', pos: 'top'}, {key: 'relations', pos: 'left'}, {key: 'cognition', pos: 'right'} ].map((item) => (
                <div key={item.key} className={`absolute w-10 h-10 rounded-full border-2 flex items-center justify-center text-[7px] font-bold ${isSilent[item.key as keyof typeof isSilent] ? 'bg-rose-200 border-rose-400' : 'bg-slate-200 border-slate-300'}`}
                  style={{ top: item.pos === 'top' ? '0' : '40%', left: item.pos === 'top' ? '25%' : item.pos === 'left' ? '0' : '50%' }}>
            ...
            <div className="relative w-20 h-20">
              {[ {key: 'interest', pos: 'top'}, {key: 'activity', pos: 'left'}, {key: 'values', pos: 'right'} ].map((item) => (
                <div key={item.key} className={`absolute w-10 h-10 rounded-full border-2 flex items-center justify-center text-[7px] font-bold ${isSilent[item.key as keyof typeof isSilent] ? 'bg-indigo-200 border-indigo-400' : 'bg-slate-200 border-slate-300'}`}
                  style={{ top: item.pos === 'top' ? '0' : '40%', left: item.pos === 'top' ? '25%' : item.pos === 'left' ? '0' : '50%' }}>
                  {PARAM_INFO[item.key].label}
                </div>
              ))}
